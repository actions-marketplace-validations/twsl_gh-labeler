...
describe("example config.yaml", () => {
  ...
  it("should have properly structured labels configuration", () => {
    ...
    if (
      config.labels?.add &&
      typeof config.labels.add === "object" &&
      !Array.isArray(config.labels.add)
    ) {
  ...
  it("should have valid action flags (close, pin, draft, etc.)", () => {
    ...
    if (
      ...
      expect(config.labels.add.wontfix?.issues?.close_reason).toBe(
        "not-planned",
      );
  ...
  it("should have valid label remove actions", () => {
    ...
    if (
      config.labels?.remove &&
      typeof config.labels.remove === "object" &&
      !Array.isArray(config.labels.remove)
    ) {
