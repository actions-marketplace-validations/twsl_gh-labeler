...
describe("AbstractHandler / BaseHandler", () => {
	...
	describe("getLabelActions", () => {
		it("should return actions for label addition", async () => {
			...
			handler = new TestHandler(mockConfig, mockActionConfig);
			const actions = await (handler as any).getLabelActions("bug", "labeled", "issue");
		...
		it("should return actions for label removal", async () => {
			...
			handler = new TestHandler(mockConfig, mockActionConfig);
			const actions = await (handler as any).getLabelActions("wip", "unlabeled", "issue");
		...
		it("should handle PR-specific actions", async () => {
			...
			handler = new TestHandler(mockConfig, mockActionConfig);
			const actions = await (handler as any).getLabelActions("needs-review", "labeled", "pr");
		...
		it("should handle discussion-specific actions", async () => {
			...
			handler = new TestHandler(mockConfig, mockActionConfig);
			const actions = await (handler as any).getLabelActions("question", "labeled", "discussion");
		...
		it("should return default actions when specific label not found", async () => {
			...
			handler = new TestHandler(mockConfig, mockActionConfig);
			const actions = await (handler as any).getLabelActions("unknown-label", "labeled", "issue");
	...
	describe("ensureUnlock", () => {
		...
		it("should re-lock even if action throws an error", async () => {
			...
			await expect((handler as any).ensureUnlock(issueParams, lock, mockAction)).rejects.toThrow("Action failed");
	...
	describe("thread type handling", () => {
		...
		it("should handle complex label configurations", async () => {
			...
			handler = new TestHandler(mockConfig, mockActionConfig);
			const actions = await (handler as any).getLabelActions("security", "labeled", "issue");
