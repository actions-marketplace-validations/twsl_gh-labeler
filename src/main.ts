...
import type Config from "@/models/internal/config"; // Import type for the loaded configuration file
...
export async function run(): Promise<void> {
	try {
		...
		const processArray = processInput
			? (processInput.split(/,|\n/).map((item) => item.trim()) as Array<"issue" | "pr" | "discussion">)
			: undefined;
		...
		const { error, value: validatedActionConfig } = ghActionConfigSchema.validate(actionConfig, { abortEarly: false });
		if (error) {
	...
	if (payload.action !== undefined && ["opened", "edited"].includes(payload.action)) {
		...
		if ("issue" in payload && payload.issue) {
			...
			core.info(`Scanning content of ${contentHandler.getThreadType()} #${threadData.number}`);
			...
			core.info(`Processing regex rules for ${threadType} #${threadData.number}`);
		...
		if ("label" in payload && payload.label && payload.action && ["labeled", "unlabeled"].includes(payload.action)) {
			...
			if ("issue" in payload && payload.issue) {
				...
				await handler.performActions(payload, threadData as DiscussionEvent["discussion"]);
			} else {
				core.info("No issue, pull request or discussion found in payload");
				return;
			}
